...
def _process_single_example_dspy(args: tuple) -> Dict[str, Any]:
    ...
    try:
        ...
        # Serialize metadata to ensure Arrow compatibility
        serialized_metadata = _serialize_metadata(usage_metadata)

        example.update(
            {"dspy_response": response_dict, "dspy_metadata": serialized_metadata}
        )
    except Exception as e:
